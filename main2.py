...
st.set_page_config(page_title="Phonics Question Generator", layout="centered")
...
def get_random_questions(age_group, level, num_questions=10):
    ...
    # ... (Rest of the get_random_questions function remains unchanged)

    # 2. Syllables
    should_exclude_syllables = is_level_1_or_2 and age_group in ["5-6", "7-8"]
    ...
    rhyming_count = 3 if is_level_1_or_2 else (2 if age_group not in ["9-10", "11-13"] else 3)
    ...
    sentences = []
    for _ in range(4):
        sentences.append(llm.get_response_from_ai(prompts.generate_text(sentences), age_group))
...
def create_thermometer_chart(metric_name, value, status, filename, max_value=100, is_count=False):
    """Create a thermometer-style chart with fixed text overlap."""
    fig, ax = plt.subplots(figsize=(4, 6))  # Large size for bold visuals
    ax.set_xlim(0, 1)
    ax.set_ylim(0, max_value + 20)
    ax.axis('off')
    
    # Thermometer outline
    ax.add_patch(Rectangle((0.4, 0), 0.2, max_value, fill=False, edgecolor='black', lw=2))
    ax.add_patch(Circle((0.5, 0), 0.15, fill=False, edgecolor='black', lw=2))
    
    # Fill based on value
    color = '#F44336' if value < 60 else '#FF9800' if value < 80 else '#34C759' if not is_count else ('#4CAF50' if value == 0 else '#F44336')
    fill_height = min(value, max_value)
    ax.add_patch(Rectangle((0.4, 0), 0.2, fill_height, fill=True, color=color))
    
    # Bulb at bottom
    ax.add_patch(Circle((0.5, 0), 0.15, fill=True, color=color))
    
    # Metric name and value (adjusted to avoid overlap)
    ax.text(0.5, max_value + 15, metric_name, ha='center', va='center', fontsize=16, weight='bold', color='black')
    ax.text(0.5, fill_height + 10 if fill_height < max_value - 10 else fill_height - 10, 
            f"{int(value)}{'' if is_count else '/100'}", ha='center', va='center', fontsize=20, weight='bold', color=color)
    ax.text(0.5, -15, status, ha='center', va='center', fontsize=12, color=color)
    
    plt.savefig(filename, bbox_inches='tight', dpi=200)  # High DPI for clarity
    plt.close()

def create_radial_chart(value, title, filename):
    """Create a radial chart for phonics skills."""
    ...
    # Background circle
    circle = plt.Circle((0, 0), 1, color='#e0e0e0')
    ax.add_patch(circle)
    ...
    # Text in center
    ax.text(0, 0, f"{int(value)}/100", ha='center', va='center', fontsize=12, weight='bold')
    
    # Title
    ax.text(0, -1.6, title, ha='center', va='center', fontsize=10)
    
    plt.savefig(filename, bbox_inches='tight', dpi=200)
    plt.close()

def create_gauge(value, filename):
    """Create a gauge for speed."""
    ...
    color = '#4CAF50' if 100 <= value <= 150 else '#FF9800' if (80 <= value < 100 or 150 < value <= 170) else '#F44336'
    status = "Excellent" if 100 <= value <= 150 else ("Too Fast" if value > 150 else "Too Slow")
    
    # Gauge background
    theta = np.linspace(np.pi, 2 * np.pi, 100)
    x = np.cos(theta)
    y = np.sin(theta)
    ax.fill(x, y, color='#e0e0e0')
    
    # Filled gauge
    gauge_percentage = min(percentage, 100)
    theta_filled = np.linspace(np.pi, np.pi + (np.pi * (gauge_percentage / 100)), 100)
    x_filled = np.cos(theta_filled)
    y_filled = np.sin(theta_filled)
    ax.fill(x_filled, y_filled, color=color)
    
    # Text
    ax.text(0, -0.2, f"{int(value)} wpm", ha='center', va='center', fontsize=14, weight='bold')
    ax.text(0, 1.6, "Speed", ha='center', va='center', fontsize=16, weight='bold')
    ax.text(0, -0.4, status, ha='center', va='center', fontsize=12, color=color)
    
    plt.savefig(filename, bbox_inches='tight', dpi=200)
    plt.close()

def create_frequency_bars(value, filename):
    """Create frequency bars for pitch (mimicking dashboard)."""
    fig, ax = plt.subplots(figsize=(4, 2))
    ax.set_xlim(0, 15)
    ax.set_ylim(0, 50)
    ax.axis('off')
    ...
    color = '#F44336' if value < 60 else '#FF9800' if value < 80 else '#34C759'
    status = "Needs Improvement" if value < 60 else "Good" if value < 80 else "Excellent"
    
    for i in range(15):
        height = base_height * (0.8 + 0.2 * abs(np.sin(i/3)) + np.random.uniform(-2, 2))
        ax.bar(i, max(5, min(50, height)), width=0.13, color=color)
    
    ax.text(7.5, 55, "Pitch", ha='center', va='center', fontsize=16, weight='bold')
    ax.text(7.5, -5, f"{int(value)}/100", ha='center', va='center', fontsize=14, weight='bold', color=color)
    ax.text(7.5, -15, status, ha='center', va='center', fontsize=12, color=color)
    
    plt.savefig(filename, bbox_inches='tight', dpi=200)
    plt.close()

def create_metric_box(metric_name, value, status, color, filename, is_small=False):
    """Create a box visualization for metrics."""
    fig, ax = plt.subplots(figsize=(3 if is_small else 4, 2 if is_small else 3))
    ax.axis('off')
    
    # Border and background
    rect = plt.Rectangle((0, 0), 1, 1, fill=True, color='#f9f9f9', ec='#2196F3', lw=2)
    ax.add_patch(rect)
    
    # Metric name
    ax.text(0.5, 0.8, metric_name, ha='center', va='center', fontsize=12 if is_small else 14, weight='bold')
    
    # Value
    ax.text(0.5, 0.5, f"{int(value)}/100" if isinstance(value, (int, float)) else str(int(value)), 
            ha='center', va='center', fontsize=16 if is_small else 20, color=color)
    
    # Status
    ax.text(0.5, 0.2, status, ha='center', va='center', fontsize=10 if is_small else 12, color=color)
...
def generate_pdf_report(child_name, age_group, metrics, questions, user_answers, pronunciation_feedbacks, final_feedback):
    ...
    # --- First Page: Title and Overall Score ---
    ...
    # Overall Score with Thermometer
    ...
    mcq_data = [["Category", "Correct (%)", "Status"]]
    for cat, count in categories.items():
        if count > 0:
            perc = (correct_counts[cat] / count * 100)
            status = "Perfect" if perc == 100 else "Needs Practice"
            mcq_data.append([cat.replace('_', ' ').title(), f"{int(perc)}/100", status])
        else:
            mcq_data.append([cat.replace('_', ' ').title(), "N/A", "Not Applicable"])
    mcq_table = Table(mcq_data)
    ...
    # Phonics Skills with Radial Charts (Horizontal)
    skills = [
    ...
    create_thermometer_chart("Intonation Score", avg_rhythm, rhythm_status, "rhythm_thermometer.png")
    create_frequency_bars(avg_pitch, "pitch_bars.png")
    create_thermometer_chart("Volume Score", avg_volume, volume_status, "volume_thermometer.png")
    
    story.append(Table([
        [Image("rhythm_thermometer.png", width=2*inch, height=3*inch), 
         Image("pitch_bars.png", width=2.5*inch, height=1.5*inch), 
         Image("volume_thermometer.png", width=2*inch, height=3*inch)]
    ...
    # --- Fluency Section ---
    story.append(Paragraph("Fluency", heading_style))
    ...
    create_thermometer_chart("Fluency Score", avg_fluency, fluency_status, "fluency_thermometer.png")
    ...
    story.append(Table([
        [Image("fluency_thermometer.png", width=2*inch, height=3*inch), 
         Image("speed_gauge.png", width=2.5*inch, height=1.5*inch), 
         Image("pause_thermometer.png", width=2*inch, height=3*inch)]
    ], colWidths=[2*inch, 2.5*inch, 2*inch]))
    story.append(Spacer(1, 12))
    ...
    story.append(Paragraph(f"Descriptions:", normal_style))
    story.append(Paragraph(f"- Fluency: {descriptions['fluency']}", normal_style))
    story.append(Paragraph(f"- Speed: {descriptions['speed']}", normal_style))
    story.append(Paragraph(f"- Pause Count: {descriptions['pause_count']}", normal_style))
    story.append(Spacer(1, 24))
    ...
    story.append(Table([
        [Image("grammar_thermometer.png", width=1.2*inch, height=2.5*inch), 
         Image("word_error_thermometer.png", width=1.2*inch, height=2.5*inch), 
         Image("verb_error_thermometer.png", width=1.2*inch, height=2.5*inch), 
         Image("article_error_thermometer.png", width=1.2*inch, height=2.5*inch), 
         Image("integrity_thermometer.png", width=1.2*inch, height=2.5*inch)]
    ...
    image_files = [
        ...
        "word_error_thermometer.png", "verb_error_thermometer.png", "article_error_thermometer.png"
...
def get_tts_text(child_name, question_index, total_questions, question_text):
    ...
    if question_number == 1:
        ...
        prompt = f"You are half way there! Let's move on to question {question_number}!"
    elif question_number == 6:
        prompt = f"This looks good {sanitized_name}! Let's move on to question {question_number}!"
    elif question_number == 7:
        prompt = f"Here comes the pronunciation round! Let's move on to question {question_number}!"
    elif question_number == 8:
        prompt = f"Great, you are mastering this! Let's move on to question {question_number}!"
    elif question_number == 9:
        prompt = f"We are almost done! Great job! Let's move on to question {question_number}!"
                    ...
                    with tab5:
                        ...
                        if len(pronunciation_questions) == 4 and len(metrics) == 4:
                            for i, (q, metric) in enumerate(zip(pronunciation_questions, metrics), 1):
                                ...
                                if word_errors or verb_errors or article_errors:
                                    if word_errors:
                                        error_html += f"<p style='color: #F44336;'>Mispronounced Words: {', '.join(word_errors)}</p>"
                                    if verb_errors:
                                        error_html += f"<p style='color: #F44336;'>Incorrect Verbs: {', '.join(verb_errors)}</p>"
                                    if article_errors:
                                        error_html += f"<p style='color: #F44336;'>Incorrect/Missing Articles: {', '.join(article_errors)}</p>"
                                else:
...
st.title("📚 Phonics Question Generator")
...
if st.session_state.show_inputs:
    ...
    age_group = st.selectbox("Select Age Group", ["5-6", "7-8", "9-10", "11-13"])
    ...
    if st.button("Generate Questions"):
        st.session_state.questions = get_random_questions(age_group, level, num_questions)
        st.session_state.user_answers = {}
        st.session_state.pronunciation_feedbacks = []
        st.session_state.current_question_index = 0
        ...
        if st.session_state[analyzed_key] and key in st.session_state.user_answers and isinstance(st.session_state.user_answers[key], dict):
                        ...
                        if st.button("Submit", key=f"submit_{i}"):
                            with st.spinner("Analyzing..."):
                                ...
                                if metrics is not None:
                                    ...
                                    st.session_state[playback_key] = False  # Reset playback state
                                    st.success("✅ Completed!")
                                else:
            ...
            # 🟢 START BUTTON - Initial state
                ...
                if st.button(f"Start Recording Q{i + 1}", key=start_key):
                    timer_placeholder.info(f"⏱️ Starting in 3 seconds...")
                    time.sleep(1)
                    timer_placeholder.info(f"⏱️ Starting in 2 seconds...")
                    time.sleep(1)
                    timer_placeholder.info(f"⏱️ Starting in a second...")
                    time.sleep(1)
            ...
            # ⛔ STOP BUTTON + TIMER - Recording state
                ...
                elapsed = int(time.time() - st.session_state[start_time_key])
                remaining = max(0, 15 - elapsed)
                timer_placeholder.info(f"⏱️ Recording Started... {remaining} seconds left")

                # Manual stop via button
                if st.button(f"Stop Recording Q{i + 1}", key=stop_key):
                    st.session_state[recording_key] = False
                    status = record_and_analyze.stop_recording()
                    st.session_state[start_time_key] = None
